...
func NewClient(ctx g.Ctx, conf *gcfg.Config) *ZincClient {
	return &ZincClient{
		ZincClientConfig: &ZincClientConfig{
			ZincHost:     conf.MustGet(ctx, "zinc.host").String(),
			ZincUser:     conf.MustGet(ctx, "zinc.user").String(),
			ZincPassword: conf.MustGet(ctx, "zinc.password").String(),
		},
	}
}
