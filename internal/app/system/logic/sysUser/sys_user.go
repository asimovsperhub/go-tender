...
func (s *sSysUser) GetAdminRules(ctx context.Context, userId uint64) (menuList []*model.UserMenus, permissions []string, roleids []uint, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		//是否超管
		isSuperAdmin := false
		//获取无需验证权限的用户id
...
func (s *sSysUser) List(ctx context.Context, req *system.UserSearchReq) (total interface{}, userList []*entity.SysUser, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		if req.UserName != "" {
			...
			m = m.Where("user_name like ?", UserName)
		}
		if req.UserNickname != "" {
			//deptIds, e := s.getSearchDeptIds(ctx, gconv.Uint64(req.DeptId))
			//liberr.ErrIsNil(ctx, e)
			UserNickname := "%" + req.UserNickname + "%"
			m = m.Where("user_nickname like ?", UserNickname)
		}
		if req.Status != "" {
		...
		err = m.FieldsEx(dao.SysUser.Columns().UserPassword, dao.SysUser.Columns().UserSalt).
			Page(req.PageNum, req.PageSize).Order("created_at desc").Scan(&userList)
...
func (s *sSysUser) Add(ctx context.Context, req *system.UserAddReq) (err error) {
	...
	req.Password = gmd5.MustEncryptString(req.Password)
	log.Println("add user passwd ", req.Password)
...
func (s *sSysUser) Edit(ctx context.Context, req *system.UserEditReq) (err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			_, err = dao.SysUser.Ctx(ctx).TX(tx).WherePri(req.UserId).Update(data)
