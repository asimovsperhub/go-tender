...
func (s *sSysAuthRule) GetIsMenuList(ctx context.Context) ([]*model.SysAuthRuleInfoRes, error) {
	...
	log.Println("GetIsMenuList-------------------->", list)
	if err != nil {
		return nil, err
	}
...
func (s *sSysAuthRule) GetMenuList(ctx context.Context) (list []*model.SysAuthRuleInfoRes, err error) {
	...
	// getMenuListFromDb从数据库中获取数据
	// GetOrSetFuncLock返回<tagKey>的值，或者将<tagKey>设置为函数<f>的结果
	// 如果<tagKey>在缓存中不存在，则返回其结果。tagKey-value对过期
	// after <duration>。如果<duration> <= 0，则它不会过期。
	//
	//注意函数<f>是在写互斥锁中执行的。
	// todo 不知道是插不进去redis还是什么问题  就先不从缓存拿数据了
	// iList := cache.GetOrSetFuncLock(ctx, consts.CacheSysAuthMenu, s.getMenuListFromDb, 0, consts.CacheSysAuthTag)
	iList, err := s.getMenuListFromDb(ctx)
	log.Println("GetMenuList---------------->", iList)
	if iList != nil {
