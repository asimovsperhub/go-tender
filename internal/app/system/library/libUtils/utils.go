...
func GetDayFirstTs() int64 {
	currentTime := time.Now()
	startTime := time.Date(currentTime.Year(), currentTime.Month(), currentTime.Day(), 0, 0, 0, 0, currentTime.Location()).Unix()
	return startTime
}

func GetDayLastTs() int64 {
	currentTime := time.Now()
	date := time.Date(currentTime.Year(), currentTime.Month(), currentTime.Day(), 23, 59, 59, 0, currentTime.Location())
	endTime := date.Unix()
	return endTime
}
...
func MonthIntervalTime(mon int) (int64, int64) {
...
func ConvertTsToDate(ts int64) string {
	formatTimeStr := time.Unix(ts, 0).Format("2006-01-02 15:04:05")
	return formatTimeStr
}

func GetDayFirstTime() string {
	return ConvertTsToDate(GetDayFirstTs())
}

func GetDayLastTime() string {
	return ConvertTsToDate(GetDayLastTs())
}
