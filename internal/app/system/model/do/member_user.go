// =================================================================================
// Code generated by GoFrame CLI tool. DO NOT EDIT.
// =================================================================================

package do

import (
	"github.com/gogf/gf/v2/frame/g"
	"github.com/gogf/gf/v2/os/gtime"
)

// MemberUser is the golang structure of table member_user for DAO operations like Where/Data.
type MemberUser struct {
	g.Meta        `orm:"table:member_user, do:true"`
	Id            interface{} //
	UserName      interface{} // 用户名
	Mobile        interface{} // 中国手机不带国家代码，国际手机号格式为：国家代码-手机号
	UserNickname  interface{} // 用户昵称
	Birthday      interface{} // 生日
	UserPassword  interface{} // 登录密码;cmf_password加密
	UserSalt      interface{} // 加密盐
	UserStatus    interface{} // 用户状态;0:禁用,1:正常,2:未验证
	UserEmail     interface{} // 用户登录邮箱
	Sex           interface{} // 性别;0:保密,1:男,2:女
	Avatar        interface{} // 用户头像
	DeptId        interface{} // 部门id
	Remark        interface{} // 备注
	MemberLevel   interface{} // 1月卡 2季卡 3 年卡   0 否
	Integral      interface{} // 积分
	Address       interface{} // 联系地址
	Describe      interface{} // 描述信息
	LastLoginIp   interface{} // 最后登录ip
	LastLoginTime *gtime.Time // 最后登录时间
	OperationId   interface{} // 操作管理员id
	CreatedAt     *gtime.Time // 创建时间
	DisabledAt    *gtime.Time // 禁用时间
	ReleaseAt     *gtime.Time // 解禁时间
	MaturityAt    *gtime.Time // 到期时间
	UpdatedAt     *gtime.Time // 更新时间
	DeletedAt     *gtime.Time // 删除时间
	DisabledDay   interface{} // 禁用天数
	Subscribe     interface{} // 可订阅数
	BuySubscribe  interface{} // 购买的订阅数
	Subscribed    interface{} // 已订阅数
}
