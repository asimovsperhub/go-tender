...
var (
	Notify = cNotifyController{}
)

type cNotifyController struct {
	BaseController
}

func (*cNotifyController) WxNotify(r *ghttp.Request) {
	...
	err = dao.VipOrder.Ctx(r.Context()).Where("order_no", orderNo).Limit(1).Scan(&order)
	...
	err = g.DB().Transaction(r.Context(), func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			_, err = dao.VipOrder.Ctx(r.Context()).Where("order_no", orderNo).Update(do.VipOrder{
			...
			_, err = dao.PurchaseLog.Ctx(r.Context()).Insert(do.PurchaseLog{
				...
				Mobile:    user.Mobile,
				Memo:      getVipOrderDesc(order.Type),
	...
	err = VipUpdateUser(r, order)
	if err != nil {
		xlog.Error("user vip update failed")
		return
	}
	r.Response.WriteJson(&wechat.V3NotifyRsp{Code: gopay.SUCCESS, Message: "成功"})
}
...
func (*cNotifyController) AlipayNotify(r *ghttp.Request) {
...
func getVipOrderDesc(orderType string) string {
